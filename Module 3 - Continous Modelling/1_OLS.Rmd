...
We can even go a step further to look at the joint effect of a number of variables on the dependent variable. We call these other variables *independent variables, x, or input variables . . .* Basically we estimate how these independent variables act together to shape the dependent variable.
...
Rule of thumb is look at the p value. You want p value to be really small(remember t tests?). So any p value below (.05) means your model is statistically significantly different than the null model.
...
Let us assume that we are interested in the effect of displacement (disp), and weight (wt) on the fuel efficiency. The formula would Change from
...
# consider having the original here to help highlight the changes
