...
Let us think of an example. Let's say you have 30 bags full of orange; While there is some difference in weights, they are more or less the same. 
...
I will now create a second batch of bags full of apples. Lets say the apples are not as uniform as oranges and they differ greatly in terms of weights. Don't worry much about this piece of code.
...
Not really. Mean is a simple model and is useful if the difference (variance) in bags is also similar between the groups. If there is great difference between the weight of the bags in the two groups, mean is not very useful (as one bag can be .7 pounds and the next may be 3.4 pounds).
...
You can see the difference between weight of Orange bags is much smaller than the difference between the weight of Apples.
...
Oranges is pretty normally distributed with a small standard deviation, whereas apples have a large standard deviation. Hence the mean value does not accurately compare apples and oranges as they are quite differently distributed.

### T Test

I want to take a tiny segue here. If you want to compare two groups and to see if they are the same or not you can use a t test (Do men and women weight the same? Are business students and engineering students have similar SAT scores?). In a normal distribution, roughly 99.7% of observations will be 2 standard deviations away from the mean. Basically, a T test compares the mean values of each group and uses the standard deviations to determine if they are indeed the same.
